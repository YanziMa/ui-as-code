...
export const animations = {
  fadeIn: (duration = "0.3s") => buildKeyframes("fadeIn", [
    { offset: "0%", properties: { opacity: "0" } },
    { offset: "100%", properties: { opacity: "1" } },
  ]),
  fadeOut: (duration = "0.3s") => buildKeyframes("fadeOut", [
  ...
  spin: (duration = "1s") => buildKeyframes("spin", [
...
export function toggleClass(el: Element, ...classes: (string | undefined | null | false)[]): void {
  for (const cls of classes) {
    ...
    if (cls === false || cls === null) {
      // Remove all if false/null passed
      el.className = "";
